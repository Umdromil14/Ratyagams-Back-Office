...
import { BASE_URL, VERSIONNING } from "../constants/enum";
...
axiosRetry(axios, {
    ...
    retryCondition: (error) => {
        const typeOfError = error.response ? error.response.status : error.code;
        switch (typeOfError) {
            case 500:
            case "ERR_NETWORK":
                return true;
 ...
 * @param {string[]} keys the keys of the object to display
...
export async function getValuesToDisplay(
    ...
    const result = valuesToString(
        (
            await axios.get(`${BASE_URL}${VERSIONNING}${url}`, {
 ...
 */
export async function getValues(url, token, params, responseType) {
...
export async function login(url, user, dispatch) {
    const { data } = await axios.post(`${BASE_URL}${VERSIONNING}${url}`, user);
    if (data.token) {
...
export async function postValues(url, token, body) {
    if (body.picture || body.platformPicture || body.videoGamesPictures) {
        await axios.post(`${BASE_URL}${VERSIONNING}${url}`, body, {
            headers: {
...
export async function deleteValues(url, token, ids) {
    await axios.delete(`${BASE_URL}${VERSIONNING}${url}/${ids.join("/")}`, {
        headers: token.headers,
    });
}
...
export async function updateValues(url, token, ids, updateValues) {
    ...
    await axios.patch(
        `${BASE_URL}${VERSIONNING}${url}/${ids.join("/")}`,
        updateValues,
        {
            headers: headers,
        }
...
export async function getAllValues(urls, token) {
    ...
    const promises = await Promise.all(
        urls.map((url) => axios.get(`${BASE_URL}${VERSIONNING}${url}`, token))
    );
    promises.map((promise) =>
        urls.map((url) => {
            if (promise.config.url === `${BASE_URL}${VERSIONNING}${url}`) {
